...
public class VertxBasedAmqpProtocolAdapterTest {
    ...
    public void testAdapterOpensSenderLinkAndNotifyDownstreamApplication() {
        ...
        final ProtonConnection connection = getConnection(null);

        adapter.handleRemoteSenderOpenForCommands(sender, connection);
    ...
    public void testAdapterClosesCommandConsumerWhenDeviceClosesReceiverLink() {
        ...
        final ProtonSender sender = getSender(sourceAddress);
        final ProtonConnection connection = getConnection(new Device(TEST_TENANT_ID, TEST_DEVICE));
    ...
    public void testUploadCommandResponseSucceeds() {
        ...
        final String reqId = Command.getRequestId("correlation-id", "reply-to-id", TEST_DEVICE);
        final String targetAddress = String.format("%s/%s/%s", CommandConstants.COMMAND_ENDPOINT,
                TEST_TENANT_ID, TEST_DEVICE, reqId, HttpURLConnection.HTTP_OK);

        final Map<String, Object> propertyMap = new HashMap<>();
        propertyMap.put(MessageHelper.APP_PROPERTY_STATUS, "200");
        propertyMap.put(AmqpAdapterConstants.APP_PROPERTY_REQUEST_ID, reqId);
        final ApplicationProperties props = new ApplicationProperties(propertyMap);
        final Message message = getFakeMessage(targetAddress);
